...
@section('content')
            ...
            <div class="page-header-content">
                <div class="row align-items-center justify-content-between pt-3">
                    ...
                    <div class="col-12 col-xl-auto mb-3">
                        <a class="btn btn-sm btn-light text-primary" href="{{ route('dokumenLainnya.index') }}">
    ...
    <div class="container-xl px-4 mt-4">
        <form
            action="@if (isset($dokumenLainnya)) {{ route('dokumenLainnya.update', $dokumenLainnya->id) }} @else {{ route('dokumenLainnya.store') }} @endif"
            method="POST" class="row" enctype="multipart/form-data">
            @csrf
            @if (isset($dokumenLainnya))
                @method('PUT')
            @endif
            ...
            <div class="col-xl-6">
                <div class="card mb-4">
                    ...
                    @if (isset($dokumenLainnya) && $dokumenLainnya->createdBy)
                        <div class="card-header bg-white">
                            <div
                                ...
                                <div class="text-xs text-muted">
                                    ...
                                    <span>
                                        {{ $dokumenLainnya->createdBy->name }}
                                    </span>
                                </div>
                                <div class="text-xs text-muted">
                                    <i class="fa-solid fa-calendar fa-xs me-1"></i>
                                    <span>
                                        {{ \Carbon\Carbon::parse($dokumenLainnya->created_at)->isoFormat('dddd, D MMMM Y H:mm') }}
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card-body">
                        @role('Superadmin|Admin|Kajur')
                            <div class="mb-3">
                                ...
                                <select name="program_studi" id="prodiField"
                                    ...
                                    <option value="SISTEM INFORMASI" @if (isset($dokumenLainnya) && $dokumenLainnya->program_studi == 'SISTEM INFORMASI') selected @endif>
                                        SISTEM INFORMASI
                                    </option>
                                    <option value="PEND. TEKNOLOGI INFORMASI" @if (isset($dokumenLainnya) && $dokumenLainnya->program_studi == 'PEND. TEKNOLOGI INFORMASI') selected @endif>
                        ...
                        <div class="mb-3">
                            ...
                            <input class="form-control @error('keterangan') is-invalid @enderror" name="keterangan"
                                ...
                                value="{{ old('keterangan', $dokumenLainnya->keterangan ?? '') }}" autocomplete="off" />
                            @error('keterangan')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        </div>

                        @if (!empty($dokumenLainnya->dokumen) && empty($dokumenLainnya->link_dokumen))
                            <div class="mb-3">
                                <label class="small mb-1 d-flex align-items-center justify-content-between">
                                    <div>Dokumen</div>
                                    <a href="/storage/repositori/dokumen-lainnya/{{ $dokumenLainnya->dokumen }}"
                            ...
                            <div class="mb-3">
                                ...
                                <div class="input-group input-group-joined">
                                    ...
                                    <input class="form-control ps-0  @error('link_dokumen') is-invalid @enderror"
                                        ...
                                        value="{{ old('link_dokumen', $dokumenLainnya->link_dokumen ?? '') }}">
                                </div>
                    ...
                    @if (isset($dokumenLainnya) && $dokumenLainnya->updatedBy)
                            ...
                            <div
                                ...
                                <div>
                                    ...
                                    <span class="fw-bolder">
                                        {{ $dokumenLainnya->updatedBy->name }}
                                    </span>
                                    pada
                                    <span class="fw-bolder">
                                        {{ \Carbon\Carbon::parse($dokumenLainnya->updated_at)->isoFormat('dddd, D MMMM Y') }}
                                    </span>
                                    pukul
                                    <span class="fw-bolder">
                                        {{ \Carbon\Carbon::parse($dokumenLainnya->updated_at)->isoFormat('H:mm') }}
                                    </span>
                                </div>
                            </div>
                        </div>
