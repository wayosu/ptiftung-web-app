...
@section('content')
            ...
            <div class="page-header-content">
                <div class="row align-items-center justify-content-between pt-3">
                    ...
                    <div class="col-12 col-xl-auto mb-3">
                        <a class="btn btn-sm btn-light text-primary" href="{{ route('lowonganKerja.index') }}">
    ...
    <div class="container-xl px-4 mt-4">
        <form
            action="@if (isset($lowonganKerja)) {{ route('lowonganKerja.update', $lowonganKerja->id) }} @else {{ route('lowonganKerja.store') }} @endif"
            method="POST" class="row" enctype="multipart/form-data">
            @csrf
            @if (isset($lowonganKerja))
                @method('PUT')
            @endif
                ...
                <div class="col-xl-4">
                    <div class="card mb-4">
                        ...
                        <div class="card-body p-0 overflow-hidden text-center">
                            <img src="{{ asset('storage/mahasiswa-dan-alumni/peluang-mahasiswa/lowongan-kerja/' . $lowonganKerja->gambar) }}"
            ...
            <div class="{{ isset($lowonganKerja) ? 'col-xl-8' : 'col-xl-6' }}">
                <div class="card mb-4">
                    <div class="card-header">Form {{ $title ?? '' }}</div>
                    @if (isset($lowonganKerja) && $lowonganKerja->createdBy)
                        <div class="card-header bg-white">
                            <div
                                ...
                                <div class="text-xs text-muted">
                                    ...
                                    <span>
                                        {{ $lowonganKerja->createdBy->name }}
                                    </span>
                                </div>
                                <div class="text-xs text-muted">
                                    <i class="fa-solid fa-calendar fa-xs me-1"></i>
                                    <span>
                                        {{ \Carbon\Carbon::parse($lowonganKerja->created_at)->isoFormat('dddd, D MMMM Y H:mm') }}
                                    </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="card-body">
                        <div class="mb-3">
                            ...
                            <input class="form-control @error('judul') is-invalid @enderror" name="judul" id="judulField"
                                ...
                                value="{{ old('judul', $lowonganKerja->judul ?? '') }}" />
                            @error('judul')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="summernote" name="deskripsi" class="form-control @error('deskripsi') is-invalid @enderror">{{ old('deskripsi', $lowonganKerja->deskripsi ?? '') }}</textarea>
                            @error('deskripsi')
                                <div class="text-danger small mt-1">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    @if (isset($lowonganKerja) && $lowonganKerja->updatedBy)
                            ...
                            <div
                                ...
                                <div>
                                    ...
                                    <span class="fw-bolder">
                                        {{ $lowonganKerja->updatedBy->name }}
                                    </span>
                                    pada
                                    <span class="fw-bolder">
                                        {{ \Carbon\Carbon::parse($lowonganKerja->updated_at)->isoFormat('dddd, D MMMM Y') }}
                                    </span>
                                    pukul
                                    <span class="fw-bolder">
                                        {{ \Carbon\Carbon::parse($lowonganKerja->updated_at)->isoFormat('H:mm') }}
                                    </span>
                                </div>
                            </div>
                        </div>
