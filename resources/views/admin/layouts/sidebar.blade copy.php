    ...
    <div class="sidenav-menu">
        <div class="nav accordion" id="accordionSidenav">
            ...
            @php
                ...
                $isSuperAdmin = $user->memilikiPeran('admin') && $user->memilikiHakIstimewa('super-admin');
                $isDosenSuperAdmin = $user->memilikiPeran('dosen') && $user->memilikiHakIstimewa('super-admin');

                $isAdmin = $user->memilikiPeran('admin') && $user->hak_istimewa === null;
                $isDosenAdmin = $user->memilikiPeran('dosen') && $user->memilikiHakIstimewa('dosen-admin');
            @endphp

            {{-- Sidenav Menu Heading - Data Master --}}
            @if ($isSuperAdmin || $isDosenSuperAdmin)
            ...
            @if ($isSuperAdmin || $isDosenSuperAdmin)
                <a class="nav-link @if (isset($active) && $active == 'kegiatan-mahasiswa') active @endif"
            ...
            @if ($dataKegiatanMahasiswas->count() > 0)
                    ...
                    <nav class="sidenav-menu-nested nav accordion" id="accordionSidenavLayout">
                        @foreach ($dataKegiatanMahasiswas as $dataKegiatanMahasiswa)
                            <a class="nav-link {{ isset($active) && $active == $dataKegiatanMahasiswa->slug ? 'active' : '' }}"
                                href="{{ route('lakm.index', $dataKegiatanMahasiswa->slug) }}">{{ $dataKegiatanMahasiswa->nama_kegiatan }}</a>
                        @endforeach
                    </nav>
                </div>
            @endif
            ...
            @role('admin|dosen')
                {{-- Sidenav Menu Heading - Informasi --}}
                ...
                {{-- Start Fasilitas --}}
                @if ($isAdmin || $isDosenAdmin || $isSuperAdmin || $isDosenSuperAdmin)
                ...
                @if (Auth::user()->memilikiPeran('dosen') && Auth::user()->hak_istimewa === null)
                ...
                {{-- Sidenav Menu Heading - Pengaturan --}}
