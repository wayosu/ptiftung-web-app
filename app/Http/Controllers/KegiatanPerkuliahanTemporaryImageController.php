...
class KegiatanPerkuliahanTemporaryImageController extends Controller
{
        ...
        try {
            ...
            if ($request->hasFile('file')) {
                foreach ($request->file('file') as $key => $value) {
                    ...
                    $storePath = 'akademik/kegiatan-perkuliahan/tmp';
                    ...
                    KegiatanPerkuliahanTemporaryImage::create([
                        'file' => $image,
    ...
    public function getTemporaryImage()
    {
        try {
            ...
            $kegiatanPerkuliahanTemporaryImages = KegiatanPerkuliahanTemporaryImage::where('user_id', $user_id)->pluck('file')->toArray();

            // Inisialisasi larik kosong untuk menyimpan detail file
            $data = [];

            // Menetapkan jalur dasar untuk gambar sementara
            $basePath = storage_path('app/public/akademik/kegiatan-perkuliahan/tmp/');

            // Lakukan iterasi pada setiap nama file yang diperoleh dari database
            foreach ($kegiatanPerkuliahanTemporaryImages as $file) {
                ...
                if (file_exists($filePath)) {
                    ...
                    $obj = [
                        ...
                        'path' => url('storage/akademik/kegiatan-perkuliahan/tmp/' . $file),
                    ];
    ...
    public function deleteTemporaryImage(Request $request)
    {
        try { // jika berhasil menghapus file
            ...
            $temporaryImage = KegiatanPerkuliahanTemporaryImage::where('file', $filename)->first();

            // jika data ditemukan
            if ($temporaryImage) {
                // menghapus file dari direktori tmp
                if (Storage::exists('akademik/kegiatan-perkuliahan/tmp/' . $temporaryImage->file)) {
                    Storage::delete('akademik/kegiatan-perkuliahan/tmp/' . $temporaryImage->file);
                }
