...
class PrasaranaTemporaryImageController extends Controller
{
        ...
        try {
            ...
            if ($request->hasFile('file')) {
                foreach ($request->file('file') as $key => $value) {
                    ...
                    $storePath = 'fasilitas/prasarana/tmp';
                    ...
                    PrasaranaTemporaryImage::create([
                        'file' => $image,
    ...
    public function getTemporaryImage()
    {
        try {
            ...
            $prasaranaTemporaryImages = PrasaranaTemporaryImage::where('user_id', $user_id)->pluck('file')->toArray();

            // Inisialisasi larik kosong untuk menyimpan detail file
            $data = [];

            // Menetapkan jalur dasar untuk gambar sementara
            $basePath = storage_path('app/public/fasilitas/prasarana/tmp/');

            // Lakukan iterasi pada setiap nama file yang diperoleh dari database
            foreach ($prasaranaTemporaryImages as $file) {
                ...
                if (file_exists($filePath)) {
                    ...
                    $obj = [
                        ...
                        'path' => url('storage/fasilitas/prasarana/tmp/' . $file),
                    ];
    ...
    public function deleteTemporaryImage(Request $request)
    {
        try { // jika berhasil menghapus file
            ...
            $temporaryImage = PrasaranaTemporaryImage::where('file', $filename)->first();

            // jika data ditemukan
            if ($temporaryImage) {
                // menghapus file dari direktori tmp
                if (Storage::exists('fasilitas/prasarana/tmp/' . $temporaryImage->file)) {
                    Storage::delete('fasilitas/prasarana/tmp/' . $temporaryImage->file);
                }
